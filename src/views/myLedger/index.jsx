...
const AccountStatement = () => {
  ...
  const [todayPLOption, setTodayPLOption] = useState('');
  ...
  const handleApplyChange = (event) => {
    ...
    // Handle apply action based on selected option if needed
  };

  const handleTodayPLChange = (event) => {
    setTodayPLOption(event.target.value);
    // Handle today P/L action based on selected option if needed
  };
  ...
  return (
    <div className="container mt-0">
      ...
      <h1 className="text-left title_h1">My Ledger</h1>

      {/* Create and Update Buttons on the Right */}
      <div className="d-flex justify-content-between align-items-center mb-3">
      <div className="d-flex">
        <h4 style={{ color: 'green' }}>Lena: 0.00</h4>
      </div>
      <div className="ml-2">
        <h4 style={{ color: 'red' }}>Dena: 0.00</h4>
      </div>
      <h4 style={{ color: 'green' }}>Balance: 0.00 (Lena)</h4>
    </div>
